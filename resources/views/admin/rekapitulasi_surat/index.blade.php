    ...
    <script>
        $(document).ready(function() {
            ...
            $('#filter-fakultas').on('change', function() {
                ...
                if (fakultasId) {
                    $.get('{{ route("admin.prodi.byFakultas", ":id") }}'.replace(':id', fakultasId), function(data) {
                        data.forEach(function(p) {
                            $prodi.append(`<option value="${p.id_prodi}">${p.nama_prodi}</option>`);
                        });
                        $prodi.trigger('change.select2');
                    });
                } else {
                    @foreach ($listProdi as $prodi)
                        $prodi.append('<option value="{{ $prodi->id_prodi }}">{{ $prodi->nama_prodi }}</option>');
                    @endforeach
                    $prodi.trigger('change.select2');
                }
            });
            ...
            let table = $('#rekap-table').DataTable({
                ...
                columns: [
                    ...
                    { data: 'fakultas', name: 'fakultas', searchable: true },
            ...
            $('#btn-export-excel').on('click', function(e) {
                e.preventDefault();
                window.location.href = '{{ route("admin.rekapitulasi.exportExcel") }}?' + getFilterParams();
            });
            ...
            $('#btn-download-bulk').on('click', function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.location.href = '{{ route("admin.rekapitulasi.downloadBulk") }}?' + getFilterParams();
                    }
                });
            });
        });
