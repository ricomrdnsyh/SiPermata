                    ...
                    <div class="card-body p-lg-8">
                        <div class="d-flex flex-column">
                            ...
                            <div id="form-container" class="mt-2">
                                <form id="kt_ecommerce_settings_general_form" class="form fv-plugins-bootstrap5 fv-plugins-framework" method="POST" action="{{ route('admin.surat-pkl.store') }}">
                                    @csrf
                                    <div class="row">
                                        ...
                                        <div class="col-12 col-md-6">
                                            <div class="fv-row mb-3">
                                                ...
                                                <div class="input-group input-group-sm">
                                                    ...
                                                    <input id="tgl_mulai" type="text" name="tgl_mulai" class="form-control form-control-sm" placeholder="Pilih tanggal mulai" autocomplete="off" value="{{ old('tgl_mulai') }}" required />
                                                </div>
                                        ...
                                        <div class="col-12 col-md-6">
                                            <div class="fv-row mb-3">
                                                ...
                                                <div class="input-group input-group-sm">
                                                    <span class="input-group-text"><i class="fas fa-calendar-alt fs-5"></i></span>
                                                    <input id="tgl_selesai" type="text" name="tgl_selesai" class="form-control form-control-sm" placeholder="Pilih tanggal selesai" autocomplete="off" value="{{ old('tgl_selesai') }}" required />
                                                </div>
                                        ...
                                        @include('admin.surat_pkl._anggota_kelompok', ['anggotaKelompok' => old('anggota_kelompok', []), 'mahasiswa' => $mahasiswa])
                                    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function buildMahasiswaOptions(selectedNim) {
                ...
                mahasiswaOptions.forEach(function(item) {
                    html += '<option value="' + item.nim + '" data-nama="' + item.nama + '" data-prodi="' + item.prodi + '"' + (selectedNim === item.nim ? ' selected' : '') + '>' + item.nim + ' - ' + item.nama + '</option>';
                });
                return html;
            }
            ...
            function validateAnggotaRows() {
                ...
                anggotaContainer.querySelectorAll('tr').forEach(function(row) {
                    const s = row.querySelector('.anggota-nim-select'); const nim = s.value;
                    syncAnggotaRow(row);
                    if (!nim) return;
                    if (ketuaNim && nim === ketuaNim) { setAnggotaError(row, 'Mahasiswa anggota tidak boleh sama dengan ketua pengaju.'); valid = false; return; }
                    if (selected.has(nim)) { setAnggotaError(row, 'Mahasiswa anggota tidak boleh duplikat.'); valid = false; return; }
                    selected.add(nim);
                });
                return valid;
            }

            window.addAnggotaKelompokAdminRow = function() { if (!anggotaContainer) return; anggotaContainer.insertAdjacentHTML('beforeend', buildAnggotaRow(anggotaIndex)); const row = anggotaContainer.lastElementChild; anggotaIndex += 1; initSelect2(row.querySelector('.anggota-nim-select')); syncAnggotaRow(row); };
