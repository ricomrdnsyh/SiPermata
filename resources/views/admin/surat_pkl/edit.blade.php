...
@section('content')
    @php
        $mahasiswaOptions = $mahasiswa
            ->map(fn($mhs) => ['nim' => $mhs->nim, 'nama' => $mhs->nama, 'prodi' => $mhs->prodi?->nama_prodi ?? '-'])
            ->values();
    @endphp
                        ...
                        <div class="card-body p-lg-8">
                            <div class="d-flex flex-column">
                                <div class="mb-6 text-center">
                                    ...
                                    <div class="text-gray-400 fw-bold fs-5">Mohon untuk perbarui semua data dengan benar.
                                    </div>
                                </div>
                                ...
                                <div id="form-container" class="mt-2">
                                    <form id="kt_ecommerce_settings_general_form"
                                        ...
                                        action="{{ route('admin.surat-pkl.update', $surat->id_surat_pkl) }}">
                                        @csrf
                                        @method('PUT')
                                        <div class="row">
                                            <div class="col-12 col-md-6">
                                                <div class="fv-row mb-3">
                                                    ...
                                                    <select class="form-select form-select-sm w-100" data-control="select2"
                                                        data-placeholder="Pilih Mahasiswa" name="nim"
                                            ...
                                            @include('admin.surat_pkl._anggota_kelompok', [
                                                'anggotaKelompok' => old(
                                                    'anggota_kelompok',
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const fpMulai = flatpickr(mulaiEl, {
                ...
                allowInput: true,
                defaultDate: mulaiEl.value || null,
                minDate: "today",
                maxDate: selesaiEl.value || null,
                onReady: function(s, d, i) {
                    if (i.altInput) {
                        i.altInput.required = true;
                        i.altInput.placeholder = mulaiEl.placeholder || '';
                    }
                },
                onChange: function(s, d) {
                    fpSelesai.set("minDate", d || null);
                }
            });
            const fpSelesai = flatpickr(selesaiEl, {
                ...
                altInputClass: "form-control form-control-sm",
                allowInput: true,
                defaultDate: selesaiEl.value || null,
                minDate: mulaiEl.value || "today",
                onReady: function(s, d, i) {
                    if (i.altInput) {
                        i.altInput.required = true;
                        i.altInput.placeholder = selesaiEl.placeholder || '';
                    }
                },
                onChange: function(s, d) {
                    fpMulai.set("maxDate", d || null);
                }
            });
            ...
            function initSelect2(element) {
                ...
                if ($el.hasClass('anggota-nim-select')) {
                    ...
                    $el.on('change.anggotaAdminSync select2:select.anggotaAdminSync select2:clear.anggotaAdminSync',
                        function() {
            ...
            function buildMahasiswaOptions(selectedNim) {
                ...
                mahasiswaOptions.forEach(function(item) {
                    html += '<option value="' + item.nim + '" data-nama="' + item.nama + '" data-prodi="' +
                        item.prodi + '"' + (selectedNim === item.nim ? ' selected' : '') + '>' + item.nim +
                        ' - ' + item.nama + '</option>';
                });
                return html;
            }
            ...
            function buildAnggotaRow(index) {
                return '<tr><td class="ps-3"><select name="anggota_kelompok[' + index +
                    '][nim]" class="form-select form-select-sm anggota-nim-select" data-control="select2" data-placeholder="Pilih mahasiswa">' +
                    buildMahasiswaOptions('') +
                    '</select><div class="invalid-feedback anggota-nim-feedback"></div></td><td><input type="text" class="form-control form-control-sm form-control-solid anggota-autofill-input anggota-nama-input" placeholder="Nama mahasiswa" disabled /><input type="hidden" name="anggota_kelompok[' +
                    index +
                    '][nama]" class="anggota-nama-hidden-input" /></td><td><input type="text" class="form-control form-control-sm form-control-solid anggota-autofill-input anggota-prodi-input" placeholder="Prodi mahasiswa" disabled /><input type="hidden" name="anggota_kelompok[' +
                    index +
                    '][prodi]" class="anggota-prodi-hidden-input" /></td><td class="pe-4 text-center"><button type="button" class="btn btn-icon btn-sm btn-light-danger remove-anggota-kelompok hover-elevate-up" title="Hapus anggota"><i class="fas fa-trash-alt"></i></button></td></tr>';
            }
            ...
            function setAnggotaError(row, msg) {
                const s = row.querySelector('.anggota-nim-select');
                const f = row.querySelector('.anggota-nim-feedback');
                s.classList.toggle('is-invalid', Boolean(msg));
                f.textContent = msg || '';
            }
            ...
            const simptUrl = "{{ route('admin.surat-pkl.simpt', '__NIM__') }}";
            ...
            if (pengajuSelect) {
                ...
                if (window.jQuery) {
                    $(pengajuSelect).on('change', function() {
            ...
            if (anggotaContainer) {
                anggotaContainer.querySelectorAll('.anggota-nim-select').forEach(function(s) {
                    initSelect2(s);
                    syncAnggotaRow(s.closest('tr'));
                });
                anggotaContainer.addEventListener('change', function(e) {
                    const s = e.target.closest('.anggota-nim-select');
                    if (s) syncAnggotaRow(s.closest('tr'));
                });
                anggotaContainer.addEventListener('click', function(e) {
                    const b = e.target.closest('.remove-anggota-kelompok');
                    if (b) {
