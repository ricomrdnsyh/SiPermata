...
class AdminEligibleLulusController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.eligible_lulus.index', compact(
            'listFakultas',
            'listProdi',
            'listTahunAkademik',
    ...
    public function getData(Request $request)
    {
        ...
        $query = MahasiswaEligibleLulus::with(['mahasiswa.prodi', 'akademik', 'addedByUser', 'fakultas']);

        if ($request->filled('fakultas_filter')) {
            $query->where('fakultas_id', $request->input('fakultas_filter'));
        }
        ...
        return DataTables::of($query)
            ...
            ->filterColumn('fakultas', function ($query, $keyword) {
                $query->whereHas('fakultas', function ($q) use ($keyword) {
    ...
    public function store(Request $request)
    {
        ...
        if (!$mahasiswa) {
            return back()->with('failed', 'NIM tidak ditemukan.');
        }
