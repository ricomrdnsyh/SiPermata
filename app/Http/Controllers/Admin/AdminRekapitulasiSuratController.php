...
class AdminRekapitulasiSuratController extends Controller
{
    protected $listSurat = [
    ...
    public function index()
    {
        ...
        $totalSelesai = HistoryPengajuan::where('status', 'selesai')->count();
        $breakdownSurat = [];
        foreach ($this->listSurat as $tabel => $nama) {
            $breakdownSurat[$nama] = HistoryPengajuan::where('status', 'selesai')
                ->where('tabel', $tabel)->count();
        }
    ...
    public function getData(Request $request)
    {
        ...
        return DataTables::of($query)
            ...
            ->filterColumn('nama_mahasiswa', fn($q, $kw) => $q->whereHas('mahasiswa', fn($s) => $s->where('nama', 'like', "%{$kw}%")))
            ->filterColumn('prodi', fn($q, $kw) => $q->whereHas('mahasiswa.prodi', fn($s) => $s->where('nama_prodi', 'like', "%{$kw}%")))
            ->filterColumn('fakultas', fn($q, $kw) => $q->whereHas('mahasiswa.prodi.fakultas', fn($s) => $s->where('nama_fakultas', 'like', "%{$kw}%")))
            ->addColumn('nama_mahasiswa', fn($r) => $r->mahasiswa?->nama ?? $r->nim)
            ->addColumn('fakultas', fn($r) => $r->mahasiswa?->prodi?->fakultas?->nama_fakultas ?? '-')
    ...
    public function exportExcel(Request $request)
    {
        ...
        $title = "Rekapitulasi Surat — Admin";
        $fileName = 'Rekapitulasi_Surat_Admin_' . date('Y-m-d') . '.xlsx';

        return Excel::download(new RekapitulasiSuratExport($data, $title, true), $fileName);
    }
    ...
    public function downloadBulkPdf(Request $request)
    {
        ...
        $zipPath = $tempDir . DIRECTORY_SEPARATOR . 'Rekapitulasi_Surat_Admin_' . date('Y-m-d') . '.zip';
        $zip = new ZipArchive();
    ...
    private function applyFilters($query, Request $request)
    {
        ...
        if ($request->filled('fakultas_filter')) {
            $query->whereHas('mahasiswa', fn($q) => $q->where('fakultas_id', $request->input('fakultas_filter')));
        }
